            ...
            <div class="flex items-center grid grid-cols-12 gap-6">
                ...
                <div class="col-span-3">
                    <div class="flex justify-end">
                        <a href="{{ route('llm-contexts.create') }}" class="btn btn-primary">
                            <i class="ti ti-plus me-1"></i> {{ __('app.add_llm_context') }}
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    <h3 class="card-title mb-4 flex justify-between items-center">
                        <span>{{ __('app.llm_context_data') }}</span>
                    </h3>
                        ...
                        <table class="table-auto w-full text-left border-spacing-0 border-separate">
                            <thead>
                                <tr>
                                    ...
                                    <th class="px-4 py-3 border-b font-semibold text-gray-800 dark:text-gray-200">{{ __('app.title') }}</th>
                            ...
                            <tbody>
                                @forelse ($llmContexts as $index => $context)
                                    <tr>
                                        <td class="px-4 py-3 border-b text-gray-500 dark:text-gray-400">{{ $index + $llmContexts->firstItem() }}</td>
                                        <td class="px-4 py-3 border-b text-gray-500 dark:text-gray-400">{{ $context->title }}</td>
                                        <td class="px-4 py-3 border-b text-gray-500 dark:text-gray-400">{{ \Illuminate\Support\Str::limit($context->context, 100) }}</td>
                                        <td class="px-4 py-3 border-b text-gray-500 dark:text-gray-400">{{ $context->created_at->format('d M Y, H:i') }}</td>
                                        <td class="px-4 py-3 border-b text-gray-500 dark:text-gray-400">
                                            <div class="flex gap-2">
                                                <a href="{{ route('llm-contexts.show', $context->id) }}" 
                                                ...
                                                <a href="{{ route('llm-contexts.edit', $context->id) }}" 
                                                ...
                                                <form action="{{ route('llm-contexts.destroy', $context->id) }}" method="POST" id="delete-form-{{ $context->id }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" 
                                                        class="p-2 bg-lighterror dark:bg-darkerror rounded-full hover:bg-error hover:text-white transition-all" 
                                                        title="{{ __('app.delete') }}"
                                                        onclick="openConfirmationModal('delete', '{{ __('app.delete_llm_context') }}', '{{ __('app.confirm_delete_llm_context') }}', '{{ __('app.yes_delete') }}', 'document.getElementById(\'delete-form-{{ $context->id }}\').submit()')">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-3 border-b text-center text-gray-500 dark:text-gray-400">{{ __('app.no_data') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                        
                        @include('components.pagination', ['paginator' => $llmContexts, 'perPage' => $perPage, 'perPageOptions' => $perPageOptions])
                    </div>
                </div>
            </div>
        </div>
