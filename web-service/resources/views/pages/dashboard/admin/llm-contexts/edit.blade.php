...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12">
            <div class="card">
                <div class="card-body">
                    <h3 class="card-title mb-4">{{ __('app.llm_context_details') }}</h3>
                    ...
                    <form action="{{ route('llm-contexts.update', $llmContext->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="title" name="title" class="form-control w-full @error('title') is-invalid @enderror" value="{{ old('title', $llmContext->title) }}" required>
                                @error('title')
                                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                                @enderror
                                
                                <label for="context" class="form-label block mb-2 mt-4 font-medium text-dark dark:text-white">{{ __('app.context_template') }} <span class="text-red-500">*</span></label>
                                <textarea id="context" name="context" class="form-control w-full @error('context') is-invalid @enderror" rows="10" required>{{ old('context', $llmContext->context) }}</textarea>
                                @error('context')
                                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
