...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12">
            <div class="card">
                <div class="card-body">
                    <h3 class="card-title mb-4">{{ __('app.edit_schedule_form') }}</h3>
                    ...
                    <form action="{{ route('consultation-schedules.update', $schedule->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="program_enrollment_id" id="program_enrollment_id" 
                                ...
                                @foreach($enrollments as $enrollment)
                                    <option value="{{ $enrollment->id }}" {{ (old('program_enrollment_id', $schedule->program_enrollment_id) == $enrollment->id) ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="mb-4">
                                ...
                                <input type="datetime-local" 
                                    ...
                                    value="{{ old('schedule_date', $schedule->schedule_date->format('Y-m-d\TH:i')) }}" required>
                                @error('schedule_date')
                                    <div class="text-error text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="status" id="status" 
                                    ...
                                    @foreach($statuses as $key => $value)
                                        <option value="{{ $key }}" {{ (old('status', is_numeric($schedule->status) ? $schedule->status : array_search($schedule->status, $statuses)) == $key) ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
