...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-4 sm:gap-6">
        <div class="col-span-12">
            <div class="card">
                <div class="card-body">
                    <h3 class="card-title mb-3 sm:mb-4 text-base sm:text-lg">Form Edit Data Checkup</h3>
                    ...
                    <form action="{{ route('checkups.update', $checkup->id) }}" method="POST">
                        ...
                        <div class="mb-4 sm:mb-6">
                            ...
                            <select id="program_enrollment_id" name="program_enrollment_id" class="form-select w-full text-xs sm:text-sm @error('program_enrollment_id') border-error @enderror" required>
                                ...
                                @foreach($enrollments as $enrollment)
                                    <option value="{{ $enrollment->id }}" {{ old('program_enrollment_id', $checkup->program_enrollment_id) == $enrollment->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4 sm:mb-6">
                            ...
                            <input type="date" id="checkup_date" name="checkup_date" 
                                   value="{{ old('checkup_date', $checkup->checkup_date ? $checkup->checkup_date->format('Y-m-d') : '') }}" 
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="height" name="height" min="100" max="250" step="0.1"
                                       value="{{ old('height', $checkup->height) }}" class="form-control w-full text-xs sm:text-sm @error('height') border-error @enderror" required>
                                @error('height')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="weight" name="weight" min="30" max="200" step="0.1"
                                       value="{{ old('weight', $checkup->weight) }}" class="form-control w-full text-xs sm:text-sm @error('weight') border-error @enderror" required>
                                @error('weight')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="body_fat" name="body_fat" min="0" max="100" step="0.1"
                                       value="{{ old('body_fat', $checkup->body_fat) }}" class="form-control w-full text-xs sm:text-sm @error('body_fat') border-error @enderror" required>
                                @error('body_fat')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="belly_fat" name="belly_fat" min="0" max="100" step="0.1"
                                       value="{{ old('belly_fat', $checkup->belly_fat) }}" class="form-control w-full text-xs sm:text-sm @error('belly_fat') border-error @enderror" required>
                                @error('belly_fat')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="muscle_mass" name="muscle_mass" min="0" max="100" step="0.1"
                                       value="{{ old('muscle_mass', $checkup->muscle_mass) }}" class="form-control w-full text-xs sm:text-sm @error('muscle_mass') border-error @enderror" required>
                                @error('muscle_mass')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="bone_density" name="bone_density" min="0" max="100" step="0.1"
                                       value="{{ old('bone_density', $checkup->bone_density) }}" class="form-control w-full text-xs sm:text-sm @error('bone_density') border-error @enderror" required>
                                @error('bone_density')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="water_content" name="water_content" min="0" max="100" step="0.1"
                                       value="{{ old('water_content', $checkup->water_content) }}" class="form-control w-full text-xs sm:text-sm @error('water_content') border-error @enderror" required>
                                @error('water_content')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="calories_needs" name="calories_needs" min="500" max="5000" step="1"
                                       value="{{ old('calories_needs', $checkup->calories_needs) }}" class="form-control w-full text-xs sm:text-sm @error('calories_needs') border-error @enderror" required>
                            ...
                            <div class="mb-3 sm:mb-4">
                                ...
                                <input type="number" id="cell_age" name="cell_age" min="1" max="120" step="1"
                                       value="{{ old('cell_age', $checkup->cell_age) }}" class="form-control w-full text-xs sm:text-sm @error('cell_age') border-error @enderror" required>
                                @error('cell_age')
                                    <div class="text-error text-xs sm:text-sm mt-1">{{ $message }}</div>
                                @enderror
                            </div>
